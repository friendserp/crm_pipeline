<template>
  <div
    class="relative bg-[#233d48] text-white font-bold flex h-full flex-col justify-between transition-all duration-300 ease-in-out"
    :class="isSidebarCollapsed ? 'w-12' : 'w-[220px]'"
    style="background-color: #233d48; color: #e5e7eb;"
  >
    <div class="p-2">
    ...
    <div class="flex-1 overflow-y-auto">
      ...
      <div v-for="view in allViews" :key="view.label">
        <div
          ...
          class="mx-2 my-2 h-px border-t border-[#4b5e6a]"
        />
        <Section
          :label="view.name"
        ...
        >
          <template #header="{ opened, hide, toggle }">
            ...
            >
              <FeatherIcon
                name="chevron-right"
                class="h-4 w-4 text-white transition-all duration-300 ease-in-out"
    ...
    <div class="m-2 flex flex-col gap-1.5">
      ...
      >
        <template #icon>
          <span class="grid h-5 w-5 flex-shrink-0 place-items-center">
            <CollapseSidebar
              class="h-5 w-5 text-white duration-300 ease-in-out"
    ...
    >
      <div class="bg-[#233d48] p-6 rounded-xl shadow-xl max-w-sm w-full text-white">
        <h2 class="text-lg font-semibold mb-4">{{ __('Confirm Clear Cookies') }}</h2>
        <p class="mb-6 text-sm">{{ __('Are you sure you want to clear all cookies? This will refresh the page.') }}</p>
        <div class="flex justify-end gap-4">
          <button
            class="px-4 py-2 bg-[#4b5e6a] rounded-lg hover:bg-[#5f7685] text-white transition-colors"
            @click="showConfirmClearCookies = false"
          >
            {{ __('Cancel') }}
          </button>
          <button
            class="px-4 py-2 bg-[#ff6b6b] text-white rounded-lg hover:bg-[#ff8787] transition-colors"
            @click="forceClearAll"
          >
            {{ __('Confirm') }}
          </button>
